...
namespace Admissions_Reserve.Model
{
    public class DataService
    {
            ...
            lock (lockObject)
            {
                using (var connection = DatabaseHelper.GetConnection())
                {
                    ...
                    using (var cmd = new SQLiteCommand(query, connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@CreatedByUserId", (object)UserAuthentication.CurrentUserId ?? DBNull.Value);
                        var result = cmd.ExecuteScalar();
                        connection.Close();
                        return Convert.ToInt32(result);
                    }
        ...
        private static void AddApplicantParameters(SQLiteCommand cmd, Applicants applicant)
        {
            ...
            // BirthDate can be nullable, format only when has value
            cmd.Parameters.AddWithValue("@BirthDate", (object)applicant.BirthDate?.ToString("yyyy-MM-dd") ?? DBNull.Value);
        ...
        private static Applicants ReadApplicantFromReader(SQLiteDataReader reader)
        {
            return new Applicants
            {
                ...
                CreatedByUserId = reader["CreatedByUserId"] != DBNull.Value ? Convert.ToInt32(reader["CreatedByUserId"]) : (int?)null
            };
        }
        ...
        public static int CreateRelative(int applicantId, string inn, string relationDegree, string lastName, 
                                          ...
                                          string email, string workplace, string position, string birthPlace = "",
                                          string idNumber = "", DateTime? issueDate = null, string issuedBy = "",
                                          string departmentCode = "", string okpo = "", string personalAccount = "",
                                          string bankName = "", string fiasAddress = "", string apartment = "",
                                          bool? pravda = null, bool? defact = null)
        {
            lock (lockObject)
            {
                using (var connection = DatabaseHelper.GetConnection())
                {
                    var query = @"INSERT INTO Relatives (
                        ...
                        BirthDate, BirthPlace, Phone, Email, WorkPlace, Position, IsBlocked, BlockReason,
                        IdNumber, IssueDate, IssuedBy, DepartmentCode, Okpo, PersonalAccount,
                        BankName, FiasAddress, Apartment, Pravda, Defact,
                        CreatedAt, UpdatedAt
                    ) VALUES (
                        @ApplicantId, @Inn, @RelationDegree, @LastName, @FirstName, @Patronymic,
                        @BirthDate, @BirthPlace, @Phone, @Email, @WorkPlace, @Position, 0, NULL,
                        @IdNumber, @IssueDate, @IssuedBy, @DepartmentCode, @Okpo, @PersonalAccount,
                        @BankName, @FiasAddress, @Apartment, @Pravda, @Defact,
                        @CreatedAt, @UpdatedAt
                    ); SELECT last_insert_rowid();";
                    ...
                    using (var cmd = new SQLiteCommand(query, connection))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@BirthPlace", (object)birthPlace ?? DBNull.Value);
                        ...
                        cmd.Parameters.AddWithValue("@IssueDate", (object)issueDate?.ToString("yyyy-MM-dd") ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@IssuedBy", (object)issuedBy ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@DepartmentCode", (object)departmentCode ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@Okpo", (object)okpo ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@PersonalAccount", (object)personalAccount ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@BankName", (object)bankName ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@FiasAddress", (object)fiasAddress ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@Apartment", (object)apartment ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@Pravda", (object)(pravda ?? false ? 1 : 0));
                        cmd.Parameters.AddWithValue("@Defact", (object)(defact ?? false ? 1 : 0));
                        cmd.Parameters.AddWithValue("@CreatedAt", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                        cmd.Parameters.AddWithValue("@UpdatedAt", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        public static Relatives GetRelative(int id)
        {
            using (var connection = DatabaseHelper.GetConnection())
            {
                var query = "SELECT * FROM Relatives WHERE Id = @Id";
                using (var cmd = new SQLiteCommand(query, connection))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new Relatives
                            {
                                ...
                                PersonalAccount = reader["PersonalAccount"]?.ToString(),
                                BankName = reader["BankName"]?.ToString(),
                                FiasAddress = reader["FiasAddress"]?.ToString(),
                                Apartment = reader["Apartment"]?.ToString(),
                                Pravda = reader["Pravda"] != DBNull.Value ? Convert.ToBoolean(reader["Pravda"]) : (bool?)null,
                                Defact = reader["Defact"] != DBNull.Value ? Convert.ToBoolean(reader["Defact"]) : (bool?)null,
        ...
        public static List<Relatives> GetApplicantRelatives(int applicantId)
        {
            ...
            using (var connection = DatabaseHelper.GetConnection())
            {
                var query = "SELECT * FROM Relatives WHERE ApplicantId = @ApplicantId";
                using (var cmd = new SQLiteCommand(query, connection))
                {
                    cmd.Parameters.AddWithValue("@ApplicantId", applicantId);
                    using (var reader = cmd.ExecuteReader())
